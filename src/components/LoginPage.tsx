...
const LoginPageContent = () => {
  ...
  const validateForm = () => {
    ...
    if (!formData.password) {
      ...
      // Pour l'inscription, exiger un mot de passe robuste
        ...
        <div className="bg-white py-8 px-4 shadow sm:rounded-lg sm:px-10 auth-card">
          <form className="space-y-6 auth-form-animation" onSubmit={handleSubmit}>
            ...
            <div>
              ...
              {/* Indicateur de force du mot de passe (seulement pour l'inscription) */}
              {!isLogin && formData.password && (
                <div className="mt-2">
                  <div className="flex items-center justify-between mb-1">
                    <span className="text-xs text-gray-600">Force du mot de passe:</span>
                    <span className={`text-xs font-medium ${
                      getPasswordStrength(formData.password).label === 'Faible' ? 'text-red-600' :
                      getPasswordStrength(formData.password).label === 'Moyen' ? 'text-yellow-600' :
                      getPasswordStrength(formData.password).label === 'Bon' ? 'text-blue-600' :
                      'text-green-600'
                    }`}>
